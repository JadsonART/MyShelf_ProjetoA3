...
export function BookCard({
  ...
  const Card = (
    <div className="group flex h-full flex-col overflow-hidden rounded-lg border bg-card text-card-foreground shadow-sm">
      {item.coverUrl && (
  ...
  if (isBook(item)) {
    return (
      <a
        href={`/read/${item.id}`}
        className="block h-full rounded-lg focus:outline-none focus:ring-2 focus:ring-ring"
      >
        {Card}
      </a>
    );
