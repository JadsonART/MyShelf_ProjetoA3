...
const DEFAULT_USER_EMAIL = "testeuna@gmail.com";
const DEFAULT_USER_PASSWORD = "Una1234@";
const LEGACY_DEFAULT_PASSWORD = "Unateste123@";
...
function upgradeDefaultUserPassword() {
  const users = read<UsersMap>(LS_USERS, {});
  let changed = false;
  for (const user of Object.values(users)) {
    if (
      user.email.toLowerCase() === DEFAULT_USER_EMAIL.toLowerCase() &&
      user.password === LEGACY_DEFAULT_PASSWORD
    ) {
